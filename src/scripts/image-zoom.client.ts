// Click/keyboard-to-zoom for content images, via medium-zoom + unzoom control.
...
document.addEventListener('astro:before-swap', () => {
	...
	const clones = Array.from(document.querySelectorAll('.medium-zoom-image--opened'));
	const settle = () => clones.forEach((el) => el.dispatchEvent(new Event('transitionend')));
	settle();
	void zoom.close();
	settle();
});
