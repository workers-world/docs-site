...
发号对接详见 [counter-worker-integration.md](./counter-worker-integration.md)。
...
# npx wrangler secrets-store secret create <STORE_ID> --name NOTIFY_AUTH_TOKEN --scopes workers --remote
...
- [invest-rss-worker README](../invest-rss-worker/README.md) — 部署与 API 速查
- [email-rule-worker README](../email-rule-worker/README.md) — 普通轨规则与 LLM digest
- [notify-worker README](../notify-worker/README.md) — 通知与 digest 合并
- [docs/invest.md](./invest.md) — 需求背景与策略决策理念
- [analysis-engine-architecture.md](./analysis-engine-architecture.md) — 摘要之后的在线世界观（旁路 `Q_ANALYSIS`，不改本仓热路径语义）
