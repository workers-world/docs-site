...
>
> 方案草稿：`cloudflare_work/.cursor/plans/有状态分析引擎_481a3f3b.plan.md`
...
| 禁止买卖           | 与 `cloudflare_work/advisor-worker/prompts/advisor.md` 相同：不产出仓位、目标价、买卖                                            |
...
| 问答   | `cloudflare_work/market-qa-agent/src/agent.ts`                | DO 只存最近问答，**不持有判断** |

人还在用邮箱和脑子当世界观。顾问聚类（`cloudflare_work/advisor-worker/src/services/advice-cluster.ts`）是「当天压成 3 段」；决策台（`cloudflare_work/decision-desk-worker/src/services/run-draft-batch.ts`）是「聚完就出买卖」。两者都不是活着的论点。
...
- **不要加在 advisor**：`cloudflare_work/docs/apply-agent.md` 已定顾问是 Executor / 数据 Worker，禁止整体转 DO。
...
fire-and-forget，`try/catch` 吞错，不阻断主路径。仿 `cloudflare_work/advisor-worker/src/services/enqueue-desk-signal.ts`。

| 上游       | 时机                                                                                                 | 现有旁路（勿改语义）         | 新增                                    |
| ---------- | ---------------------------------------------------------------------------------------------------- | ---------------------------- | --------------------------------------- |
| invest-rss | LLM enrich 成功，`cloudflare_work/invest-rss-worker/src/services/llm-enrich/route-after-llm.ts` 末尾 | `bypassToDecisionDesk`       | `bypassToAnalysisEngine` → `Q_ANALYSIS` |
| advisor    | `cloudflare_work/advisor-worker/src/services/analyze-advice.ts` 成功后                               | `bypassAdviceToDecisionDesk` | 同文件旁路再投 `Q_ANALYSIS`             |

Queue consumer：校验 envelope → 经 DO binding stub 唤醒 `WorldviewAgent.ingest` → ack。LLM 只在 DO 内调用（且只在 alarm 合并窗口内调用）。**业务队列 `max_retries = 5`**——consumer 本身无 LLM，配额与 Evaluator 拒绝都发生在 DO merge 期，不占队列重试（遵循 `cloudflare_work/.cursor/rules/cloudflare-worker-queue-retry.mdc`）。
...
软评判（可选 LLM judge）不得覆盖硬规则。范式见 `cloudflare_work/docs/抽象-spec.md`。

---

## 7. API（Phase 1）

鉴权：Bearer `ANALYSIS_ADMIN_TOKEN`（Store 可与 `RULES_ADMIN_TOKEN` 同值对照，见 `cloudflare_work/docs/secrets-inventory.md`；新建时按 `[校验方域]_[用途]_TOKEN`）。
...
| 用途             | 路径                                                                                                                    |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------- |
| 顾问旁路模板     | `cloudflare_work/advisor-worker/src/services/enqueue-desk-signal.ts`                                                    |
| 分析信号类型模板 | `cloudflare_work/framework_sdk_worker/src/desk/desk-signal.ts`（仿此建 `src/analysis/analysis-signal.ts`）              |
| invest 旁路点    | `cloudflare_work/invest-rss-worker/src/services/llm-enrich/route-after-llm.ts`                                          |
| Agent / DO 模板  | `cloudflare_work/market-qa-agent/src/agent.ts`（仅参考 DO fetch/SQL 用法；本引擎用纯 DO，且是 push 更新不是 pull 问答） |
| 禁止买卖约束     | `cloudflare_work/advisor-worker/prompts/advisor.md`                                                                     |
| 顾问不转 DO      | `cloudflare_work/docs/apply-agent.md`                                                                                   |
| 决策台边界       | `cloudflare_work/docs/decision-desk-architecture.md`                                                                    |

---

## 11. 相关文档

- [投资 RSS 架构](/1_architecture/invest-rss/) — 摘要层与 LLM enrichment
- `cloudflare_work/docs/apply-agent.md` — 顾问聚类 + market-qa；本引擎是第三条路径
- `cloudflare_work/docs/decision-desk-architecture.md` — 买卖草稿
- `cloudflare_work/docs/market-qa-agent-integration.md` — 拉式问答
- `cloudflare_work/docs/抽象-spec.md` — Hybrid Evaluator
- `cloudflare_work/docs/投资顾问Agent模块需求文档.md` — 顾问单条分析（本引擎不替代）
